...
const DOMAINS: DomainItem[] = [
  ...
  },
  {
    ...
    badge: "Live Masterclass & Simulators",
    status: "live",
    themeColor: "#8C6A2E",
    ...
    internalUrl: "/finance/",
    description:
      "Core corporate finance principles, financial statement reconstruction (NOPAT/NOWC), Additional Funds Needed (AFN) forecasting, working capital cash conversion cycles, bond mechanics, CAPM hurdle rates, and multi-stage DCF valuation.",
    stats: [
      { label: "Modules", value: "8 In-Depth Tabs" },
      { label: "Simulators", value: "7 Live Dynamic Engines" },
      { label: "Curriculum", value: "IIM / Brigham & Ehrhardt" },
    ],
    frameworks: [
      "Accounting Reconstruction (NOPAT, NOWC, FCF, ROIC & EVA)",
      "Pro Forma Forecasting & Additional Funds Needed (AFN)",
      "Working Capital Policies & Cash Conversion Cycle (CCC)",
      "Time Value of Money (TVM), Annuities & Loan Amortization",
      "Bond Valuation, Yield to Maturity (YTM) & After-Tax Cost of Debt",
      "Portfolio Diversification & Capital Asset Pricing Model (CAPM)",
      "Enterprise DCF Valuation, Horizon Value & WACC Sensitivity",
      "Market Efficiency (EMH), Behavioral Biases & Corporate Governance",
    ],
    sampleFormulas: [
      { label: "Free Cash Flow (FCF)", formula: "\\text{FCF} = \\text{NOPAT} - \\text{Net Investment in Operating Capital}" },
      { label: "Additional Funds Needed (AFN)", formula: "AFN = \\left(\\frac{A_0^*}{S_0}\\right)\\Delta S - \\left(\\frac{L_0^*}{S_0}\\right)\\Delta S - M \\cdot S_1 \\cdot (1 - \\text{Payout})" },
      { label: "Capital Asset Pricing Model (CAPM)", formula: "r_s = r_{RF} + \\beta_i [r_M - r_{RF}]" },
      { label: "Corporate Valuation Model (DCF)", formula: "V_0 = \\sum_{t=1}^{N} \\frac{\\text{FCF}_t}{(1 + \\text{WACC})^t} + \\frac{HV_N}{(1 + \\text{WACC})^N}" },
    ],
    references: [
    ...
    learningOutcomes: [
      "Reconstruct GAAP accounting statements into NOPAT, NOWC, and Free Cash Flows to measure Economic Value Added (EVA).",
      "Forecast spontaneous balance sheet items and solve for Additional Funds Needed under capacity constraints.",
      "Optimize cash conversion cycles (DIO, DSO, DPO) to minimize working capital capital charges.",
      "Calculate bond prices, duration, yield to maturity, and the after-tax cost of debt.",
      "Derive stock Betas, compute CAPM cost of equity, and construct Markowitz 2-asset risk-return frontiers.",
      "Build full 5-year multi-stage DCF models with terminal growth horizon values and dual-variable sensitivity grids.",
    ],
  },
  {
    id: "marketing",
